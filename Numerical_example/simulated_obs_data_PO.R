...
plot_df <- plot_df %>%
  mutate(beta_S = factor(beta_S, levels = c(0, 2, 5, 10)))

# Plot: one panel per Method, grouped by VE_type and beta_S (categorical x-axis)
ggplot(plot_df, aes(x = beta_S, group = VE_type)) +
  geom_errorbar(aes(ymin = Lower, ymax = Upper),
                width = 0.2, position = position_dodge(width = 0.5)) +
  geom_point(aes(y = True),
             size = 2.5, position = position_dodge(width = 0.5)) +
  facet_grid(VE_type~ Method) +
  labs(title = "True VE and Bounds by Method and beta_S",
       x = expression(beta[S]),
...
ggsave("G:\\My Drive\\PhD3\\data_example\\Figures\\ve_bounds_plot.pdf", width = 6.5, height = 3, units = "in")
