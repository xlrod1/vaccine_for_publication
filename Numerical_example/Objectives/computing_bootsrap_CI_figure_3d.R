...
true_LP  <- compute_LP_formula_bounds_no_S(
  df_true,
  monotonicity_constraint = "positive_M"
)
true_mon <- compute_monotonicity_bounds_no_S(df_true)
true_LP
true_mon
...
for (i in seq_len(R)) {
  ...
  boot_mat <- replicate(B, {
    ...
    dfb   <- df_i[idx, ]
    lp_b  <- compute_LP_formula_bounds_no_S(dfb,
                                            monotonicity_constraint = "positive_M")
    mon_b <- compute_monotonicity_bounds_no_S(dfb)
