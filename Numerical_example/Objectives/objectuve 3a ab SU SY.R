...
simulate_data_PO <- function(n,beta_S,gamma_S) {
  ...
  p_S_0 <- plogis(-log(0.5) + log(1.5)*0 + beta_S*U)
  p_S_1 <- plogis(-log(0.5) + log(1.5)*1 + beta_S*U)
  S_0 <- rbinom(n, 1, p_S_0)
  S_1 <- rbinom(n, 1, p_S_1)
  S<-ifelse(A==1,S_1,S_0)
  # B depends on S and U (increasing in U)
  p_B_0 <- plogis(-log(0.5) + log(1.5)*S_0 + log(1.5)*U)
  p_B_1 <- plogis(-log(0.5)  + log(1.5)*S_1 + log(1.5)*U)
  ...
  p_Y_00<-plogis(-log(3)*0 + gamma_S*S_0 + log(1.5)*0 + log(1.1)*U)
  p_Y_01<-plogis(-log(3)*0 + gamma_S*S_0 + log(1.5)*1 + log(1.1)*U)
  p_Y_10<-plogis(-log(3)*1 + gamma_S*S_1 + log(1.5)*0 + log(1.1)*U)
  p_Y_11<-plogis(-log(3)*1 + gamma_S*S_1 + log(1.5)*1 + log(1.1)*U)
...
all_bounds <- list()
beta_vals <- c(0, log(1.5), log(2), log(3))
...
for (b in beta_vals) {
  ...
  mon       <- compute_monotonicity_bounds(df,monotonicity_constraint ="positive_M")
...
plot_df <- bind_rows(all_bounds) %>%
  mutate(
   # beta_S = factor(beta_S, levels = c(0, log(1.5), log(2), log(3))),
    Method = factor(Method, levels = c("LP", "Monotonicity")),
    Use_S = factor(Use_S, levels = c("With S", "No S"))
  )

ll<-plot_df %>% filter(VE_type=="VEt")

# check<-plot_df %>% filter(True<Lower)
# check2<-plot_df2 %>% filter(True<Lower)
...
plot_df2 <- plot_df %>%
  mutate(
    ...
    gamma_lab = sapply(beta_S, make_gamma_label)
  )



desired_order <- c( "0", "log(1.5)","log(2)","log(3)")
...
ggplot(plot_df2, aes(x = gamma_lab, group = VE_type,type=Method)) +
  ...
  labs(x = expression(delta[U]), y = "True VE and estimated bounds (%)") +
...
for (b in beta_vals) {
  df <- simulate_data_PO(1000000, beta_S = b,gamma_S=log(2))
...
# 3. Plot
plot_df2<-plot_df %>% filter(Use_S=="With S") %>% 
  filter(Lower<Upper)
